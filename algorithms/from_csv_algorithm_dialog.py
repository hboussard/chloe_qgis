...
class FromCSVParametersPanel(ChloeParametersPanel):
    ...
    def __init__(self, parent, alg):
        ...
        self.layoutMain.insertWidget(3,pb)
        self.pbHeader = pb
    ...
    def uploadHeader(self):
        ...
        dlg.setNameFilters(["All File (*.*)","ASC File (*.asc)","TXT File (*.txt)"])
        ...
        if filenames!='':
            ...
            with open(str(filenames[0]), 'r') as infile:
                for line in infile:
                    ...
                    if values[0] == "ncols":
                        ChloeUtils.wrapperSetValue(self.wrappers["N_COLS"], int(values[1]))
                    elif values[0] == "nrows":
                        ChloeUtils.wrapperSetValue(self.wrappers["N_ROWS"], float(values[1]))
                    elif values[0] == "xllcorner":
                        ChloeUtils.wrapperSetValue(self.wrappers["XLL_CORNER"], float(values[1]))
                    elif values[0] == "yllcorner":
                        ChloeUtils.wrapperSetValue(self.wrappers["YLL_CORNER"], float(values[1]))
                    elif values[0] == "cellsize":
                        ChloeUtils.wrapperSetValue(self.wrappers["CELL_SIZE"], float(values[1]))
                    elif values[0] == "NODATA_value":
                        ChloeUtils.wrapperSetValue(self.wrappers["NODATA_VALUE"], int(values[1]))
                    else:
                        break
