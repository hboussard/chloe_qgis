...
class GridMultiAlgorithmDialog(ChloeAlgorithmDialog):
    ...
    def __init__(self, alg):
        ...
        self.mainWidget = GridMultiParametersPanel(self, alg) # Main parameter windows
...
class GridMultiParametersPanel(ChloeParametersPanel):
    ...
    def __init__(self, parent, alg):
        ...
        # === param:GRID_SIZES Widget:IntListSelectionPanel ===
        lineEdit = self.widgets["GRID_SIZES"].lineEdit
        lineEdit.setReadOnly(True)
        sbInt = self.widgets["GRID_SIZES"].sbInt
        sbInt.setValue(3)
    ...
    def addIntInListDst(self):
        ...
        int_value = self.widgets["GRID_SIZES"].sbInt.value()

        
        self.grid_sizes_selected.add(str(int_value))
        

        listDest = self.widgets["GRID_SIZES"].listDest
        listDest.clear()
        listDest.addItems(list(self.grid_sizes_selected))
        self.updateGridSizesLigneEdit()
    ...
    def removeIntInListDst(self):
        ...
        selected = self.widgets["GRID_SIZES"].listDest.selectedItems()
        for lw in selected:
            self.grid_sizes_selected.remove(lw.text())
    ...
    def updateGridSizesLigneEdit(self):
        """update Gris sizes (QLineEdit in readOnly mode)"""
        lineEdit = self.widgets["GRID_SIZES"].lineEdit
        grid_sizes = list(self.grid_sizes_selected)
        if grid_sizes:
            lineEdit.setText(";".join(grid_sizes))
        else:
            lineEdit.setText("")
